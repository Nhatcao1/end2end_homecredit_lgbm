...
int main(int argc, char** argv) {
  ...
  try {
    ...
    require(Serial::SerializeToFile(argv[@FIRST_CT@], encryptedFirst, SerType::BINARY), "cannot save first ciphertext");
    require(Serial::SerializeToFile(argv[@SECOND_CT@], encryptedSecond, SerType::BINARY), "cannot save second ciphertext");
    @SAVE_VALID@
    require(Serial::SerializeToFile(argv[@RESULT_CT@], encryptedResult, SerType::BINARY), "cannot save result ciphertext");
    auto decrypted = @ENTRY@__decrypt__result0(context, encryptedResult, keys.secretKey);
    const auto done = std::chrono::steady_clock::now();
    std::ofstream audit(argv[@AUDIT@]); audit << "value\n" << std::setprecision(17);
    ...
    metrics << "{\n  \"encryption_seconds\": " << std::chrono::duration<double>(evaluationStarted-encryptionStarted).count()
            << ",\n  \"encrypted_evaluation_seconds\": " << std::chrono::duration<double>(auditStarted-evaluationStarted).count()
...
def execute(directory: Path, entry: str, size: int, first: Path, second: Path, valid: Path | None, openfhe_dir: str) -> tuple[list[float], dict[str, Any]]:
    has_valid = valid is not None
    work = directory / "runner"; work.mkdir()
    for name in ("heir_output.cpp", "heir_output.h"): shutil.copy2(directory / name, work / name)
    options = ({"@ARGC@":"10", "@READ_VALID@":"const auto valid = readVector(argv[3]); require(valid.size() == @VECTOR_SIZE@, \"bad validity size\");", "@ENCRYPT_VALID@":"auto encryptedValid = @ENTRY@__encrypt__arg2(context, valid, keys.publicKey);", "@CALL_ARGUMENTS@":"encryptedFirst, encryptedSecond, encryptedValid", "@FIRST_CT@":"4", "@SECOND_CT@":"5", "@SAVE_VALID@":"require(Serial::SerializeToFile(argv[6], encryptedValid, SerType::BINARY), \"cannot save validity ciphertext\");", "@RESULT_CT@":"7", "@AUDIT@":"8", "@METRICS@":"9"} if has_valid else {"@ARGC@":"8", "@READ_VALID@":"", "@ENCRYPT_VALID@":"", "@CALL_ARGUMENTS@":"encryptedFirst, encryptedSecond", "@FIRST_CT@":"3", "@SECOND_CT@":"4", "@SAVE_VALID@":"", "@RESULT_CT@":"5", "@AUDIT@":"6", "@METRICS@":"7"})
    ...
    ciphertexts = directory / "ciphertexts"; ciphertexts.mkdir(); audit, metrics = directory / "decrypted_audit.csv", directory / "metrics.json"
    command = [str((build / "ciphertext_runner").resolve()), str(first.resolve()), str(second.resolve())]
    if has_valid: command += [str(valid.resolve()), str(ciphertexts / "first.ct"), str(ciphertexts / "second.ct"), str(ciphertexts / "validity.ct"), str(ciphertexts / "result.ct"), str(audit.resolve()), str(metrics.resolve())]
    else: command += [str(ciphertexts / "first.ct"), str(ciphertexts / "second.ct"), str(ciphertexts / "result.ct"), str(audit.resolve()), str(metrics.resolve())]
    ...
    result.update({"build_seconds":{"configure":configure_seconds,"build":build_seconds}, "runner_wall_seconds":wall, "ciphertext_files":[str(item) for item in sorted(ciphertexts.glob("*.ct"))]})
...
def main() -> None:
    ...
    perc, perc_info = execute(perc_dir, "payment_perc_newton", args.vector_size, payment_path, installment_path, valid_path, args.openfhe_dir)
    diff, diff_info = execute(diff_dir, "encrypted_subtract", args.vector_size, installment_path, payment_path, None, args.openfhe_dir)
    ...
    result = {"status":"heir_generated_ckks_executed", "generated":generated, "execution":{"payment_perc":perc_info,"payment_diff":diff_info}, "comparison":rows, "note":"PAYMENT_PERC is approximate reciprocal CKKS; PAYMENT_DIFF is native CKKS subtraction."}
