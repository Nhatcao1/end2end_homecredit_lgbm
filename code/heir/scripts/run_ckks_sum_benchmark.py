...
int main(int argc,char**argv){if(argc!=6)return 2;try{const size_t slots=@SIZE@;const double inputScale=std::stod(argv[3]);if(!(inputScale>0.0))throw std::runtime_error("input scale must be positive");auto setup=std::chrono::steady_clock::now();auto ctx=fixed_count_mean__generate_crypto_context();auto keys=ctx->KeyGen();if(!keys.good())throw std::runtime_error("key generation failed");ctx=fixed_count_mean__configure_crypto_context(ctx,keys.secretKey);ctx=encrypted_sum__configure_crypto_context(ctx,keys.secretKey);std::ofstream meta(argv[5]);meta<<std::setprecision(17)<<"{\"setup_seconds\":"<<seconds(setup)<<",\"ring_dimension\":"<<ctx->GetRingDimension()<<",\"slot_count\":"<<slots<<",\"input_scale\":"<<inputScale<<",\"omp_num_threads\":1}\n";std::ofstream out(argv[4]);out<<"value_count,decimals,repetition,ciphertext_chunks,input_scale,encrypt_seconds,sum_evaluate_seconds,merge_seconds,mean_scale_seconds,sum_decrypt_seconds,mean_decrypt_seconds,online_seconds,he_sum,sum_abs_error,he_mean,mean_abs_error\n"<<std::setprecision(17);for(size_t count:counts(argv[2]))for(int d:{1,2,3,6}){auto rawValues=read(std::string(argv[1])+"/add_sub_"+std::to_string(count)+"_"+std::to_string(d)+"dp.csv");double plain=0;for(double x:rawValues)plain+=x;const double plainMean=plain/static_cast<double>(count);std::vector<double> values;values.reserve(rawValues.size());for(double x:rawValues)values.push_back(x/inputScale);for(int r=1;r<=5;++r){double enc=0,eval=0,merge=0;size_t chunks=0;bool first=true;decltype(encrypted_sum__encrypt__arg0(ctx,std::vector<double>(slots),keys.publicKey)) total;for(size_t start=0;start<values.size();start+=slots,++chunks){std::vector<double> block(slots,0.0);size_t take=std::min(slots,values.size()-start);std::copy(values.begin()+start,values.begin()+start+take,block.begin());auto t=std::chrono::steady_clock::now();auto input=encrypted_sum__encrypt__arg0(ctx,block,keys.publicKey);enc+=seconds(t);t=std::chrono::steady_clock::now();auto partial=encrypted_sum(ctx,input);eval+=seconds(t);t=std::chrono::steady_clock::now();if(first){total=partial;first=false;}else{total=heir::runtime::add_bundles(ctx,total,partial);}merge+=seconds(t);}auto t=std::chrono::steady_clock::now();auto mean=heir::runtime::mean_from_sum(ctx,total,count);const double meanScale=seconds(t);std::cerr<<"audit=sum count="<<count<<" decimals="<<d<<" repetition="<<r<<'\n';t=std::chrono::steady_clock::now();double sumValue=encrypted_sum__decrypt__result0(ctx,total,keys.secretKey)*inputScale;const double sumDecrypt=seconds(t);std::cerr<<"audit=mean count="<<count<<" decimals="<<d<<" repetition="<<r<<'\n';t=std::chrono::steady_clock::now();double meanValue=encrypted_sum__decrypt__result0(ctx,mean,keys.secretKey)*inputScale;const double meanDecrypt=seconds(t);out<<count<<','<<d<<','<<r<<','<<chunks<<','<<inputScale<<','<<enc<<','<<eval<<','<<merge<<','<<meanScale<<','<<sumDecrypt<<','<<meanDecrypt<<','<<enc+eval+merge+meanScale+sumDecrypt+meanDecrypt<<','<<sumValue<<','<<std::abs(sumValue-plain)<<','<<meanValue<<','<<std::abs(meanValue-plainMean)<<'\n';}}return 0;}catch(const std::exception&e){std::cerr<<e.what()<<'\n';return 1;}}
'''

def pandas_sum(data:Path, counts:tuple[int,...], input_scale:float, output:Path)->None:
    ...
    with output.open("w",newline="",encoding="utf-8") as h:
        w=csv.writer(h);w.writerow(["value_count","decimals","repetition","pandas_sum_seconds","pandas_mean_seconds","pandas_sum","pandas_mean"])
        for count in counts:
            for d in (1,2,3,6):
                values=pd.read_csv(data/f"add_sub_{count}_{d}dp.csv",usecols=["left"])["left"]
                # Match the public CKKS representation exactly. This client-side
                # normalization is setup, not part of either calculation timer.
                encoded=values/input_scale
                for r in range(1,6):
                    t=time.perf_counter(); encoded_sum=encoded.sum(); sum_seconds=time.perf_counter()-t
...
def main()->None:
    ...
    lines=[
        ...
        "",
        "Pandas and HE both compute over the same normalized `x / scale` values; the displayed result is restored to the original unit after calculation. "
        "Pandas CSV-read/normalization setup time is excluded, matching HE client encoding preparation. "
        f"One shared HE context/key setup takes `{float(execution['setup_seconds']):.9f}` s and is reported once rather than amortized into a workload ratio. "
        ...
        "",
        "| Values | Decimals | Pandas sum (s) | HE encrypt (s) | HE SUM reduction (s) | HE merge (s) | Audit decrypt (s) | SUM end-to-end online (s) | HE calc ÷ Pandas | Online ÷ Pandas | Sum max error |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for count in counts:
        for d in ("1","2","3","6"):
            ...
            pandas_seconds=statistics.median(float(r['pandas_sum_seconds']) for r in pr); calculation=(float(r["sum_evaluate_seconds"])+float(r["merge_seconds"]) for r in hr); sum_online=(float(r["encrypt_seconds"])+float(r["sum_evaluate_seconds"])+float(r["merge_seconds"])+float(r["sum_decrypt_seconds"]) for r in hr); calculation_seconds=statistics.median(calculation); online_seconds=statistics.median(sum_online)
            lines.append(f"| {count} | {d} | {pandas_seconds:.9f} | {statistics.median(float(r['encrypt_seconds']) for r in hr):.9f} | {statistics.median(float(r['sum_evaluate_seconds']) for r in hr):.9f} | {statistics.median(float(r['merge_seconds']) for r in hr):.9f} | {statistics.median(float(r['sum_decrypt_seconds']) for r in hr):.9f} | {online_seconds:.9f} | {calculation_seconds/pandas_seconds:.2f}× | {online_seconds/pandas_seconds:.2f}× | {max(float(r['sum_abs_error']) for r in hr):.12g} |")
    lines.extend([
        "",
        ...
        "",
        "| Values | Decimals | Pandas mean (s) | Inherited SUM path (s) | HE mean scale only (s) | Audit decrypt (s) | MEAN end-to-end online (s) | HE calc ÷ Pandas | Online ÷ Pandas | Mean max error |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ])
    for count in counts:
        for d in ("1","2","3","6"):
            hr=[r for r in rows if r["value_count"]==str(count) and r["decimals"]==d];pr=[r for r in prow if r["value_count"]==str(count) and r["decimals"]==d]
            pandas_seconds=statistics.median(float(r['pandas_mean_seconds']) for r in pr); inherited=(float(r["encrypt_seconds"])+float(r["sum_evaluate_seconds"])+float(r["merge_seconds"]) for r in hr); calculation=(float(r["sum_evaluate_seconds"])+float(r["merge_seconds"])+float(r["mean_scale_seconds"]) for r in hr); mean_online=(float(r["encrypt_seconds"])+float(r["sum_evaluate_seconds"])+float(r["merge_seconds"])+float(r["mean_scale_seconds"])+float(r["mean_decrypt_seconds"]) for r in hr); calculation_seconds=statistics.median(calculation); online_seconds=statistics.median(mean_online)
            lines.append(f"| {count} | {d} | {pandas_seconds:.9f} | {statistics.median(inherited):.9f} | {statistics.median(float(r['mean_scale_seconds']) for r in hr):.9f} | {statistics.median(float(r['mean_decrypt_seconds']) for r in hr):.9f} | {online_seconds:.9f} | {calculation_seconds/pandas_seconds:.2f}× | {online_seconds/pandas_seconds:.2f}× | {max(float(r['mean_abs_error']) for r in hr):.12g} |")
