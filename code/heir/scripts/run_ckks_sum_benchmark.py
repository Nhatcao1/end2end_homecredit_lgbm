...
def python_sum(data: Path, count: int, output: Path) -> None:
    with output.open("w",newline="",encoding="utf-8") as h:
        w=csv.writer(h);w.writerow(["decimals","repetition","python_seconds"])
        for d in (1,2,3,6):
            with (data/f"add_sub_{count}_{d}dp.csv").open(newline="",encoding="utf-8") as src: values=[float(r["left"]) for r in csv.DictReader(src)]
            for r in range(1,6): t=time.perf_counter();sum(values);w.writerow([d,r,time.perf_counter()-t])
...
def main() -> None:
    ...
    lines=["# CKKS-SUM-01", "", "| Decimals | Python sum median (s) | HE evaluation median (s) | HE encrypt median (s) | HE decrypt median (s) | HE online median (s) | Max absolute error |", "|---:|---:|---:|---:|---:|---:|---:|"]
    for d in ("1","2","3","6"):
