...
import argparse, csv, json, re, shutil, statistics, subprocess, sys, time
...
def patch_multiply_depth(source, requested_depth):
 if requested_depth < 2:raise ValueError('--ckks-mul-depth must be at least 2')
 match=re.search(r'(SetMultiplicativeDepth\()\d+(\s*\);)',source)
 if match is None:raise ValueError('generated multiply source has no SetMultiplicativeDepth call')
 original=int(re.search(r'\d+',match.group(0)).group(0));patched,count=re.subn(r'(SetMultiplicativeDepth\()\d+(\s*\);)',rf'\g<1>{requested_depth}\g<2>',source)
 if count!=1:raise ValueError(f'expected one multiply depth setting; found {count}')
 return patched,original

def emitted_parameter(source,name):
 match=re.search(rf'{re.escape(name)}\((\d+)\);',source)
 return int(match.group(1)) if match else 'HEIR default (not explicitly emitted)'

def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--generated-dir',type=Path,required=True);p.add_argument('--prepared-dir',type=Path,default=Path('data/prepared/installments_columns'));p.add_argument('--value-count',dest='value_counts',nargs='+',type=int,default=[1000]);p.add_argument('--input-scale',type=float,default=0);p.add_argument('--repetitions',type=int,default=5);p.add_argument('--ckks-mul-depth',type=int,default=3,help='shared multiply-capable CKKS depth; default: 3');p.add_argument('--output-dir',type=Path,required=True);p.add_argument('--openfhe-dir',default='/usr/local/lib/OpenFHE');p.add_argument('--overwrite',action='store_true');a=p.parse_args();root=a.output_dir.resolve()
 if len(a.value_counts)>1:
  ...
  for count in a.value_counts:
   ...
   command=[sys.executable,str(Path(__file__).resolve()),'--generated-dir',str(a.generated_dir.resolve()),'--prepared-dir',str(a.prepared_dir.resolve()),'--value-count',str(count),'--input-scale',str(a.input_scale),'--repetitions',str(a.repetitions),'--ckks-mul-depth',str(a.ckks_mul_depth),'--output-dir',str(child),'--openfhe-dir',a.openfhe_dir,'--overwrite']
 ...
 mul_cpp=work/'mul_output.cpp';patched_mul,original_mul_depth=patch_multiply_depth(mul_cpp.read_text(encoding='utf-8'),a.ckks_mul_depth);mul_cpp.write_text(patched_mul,encoding='utf-8');emitted_multiply_parameters={'multiplicative_depth':emitted_parameter(patched_mul,'SetMultiplicativeDepth'),'first_mod_size':emitted_parameter(patched_mul,'SetFirstModSize'),'scaling_mod_size':emitted_parameter(patched_mul,'SetScalingModSize')}
 (work/'primitive_runner.cpp').write_text(RUNNER.replace('@SLOTS@','8192'),encoding='utf-8');(work/'CMakeLists.txt').write_text(CMAKE,encoding='utf-8');build=work/'build';run(['cmake','-S',str(work),'-B',str(build),f'-DOpenFHE_DIR={a.openfhe_dir}'],work);run(['cmake','--build',str(build),'--target','primitive_runner'],work);he=root/'heir_results.csv';meta=root/'execution.json';wall,log=run(['env','OMP_NUM_THREADS=1',str((build/'primitive_runner').resolve()),str(stage.resolve()),str(scale),str(a.repetitions),str(he.resolve()),str(meta.resolve())],work);(work/'runner.log').write_text(log,encoding='utf-8');rows=list(csv.DictReader(he.open()));python_rows=list(csv.DictReader(python_csv.open()));execution=json.loads(meta.read_text(encoding='utf-8'));execution['multiply_context_budget']={'translated_depth_before_patch':original_mul_depth,'requested_multiplicative_depth':a.ckks_mul_depth,'emitted_parameters':emitted_multiply_parameters};write_json(meta,execution);lines=['# Real installments CKKS primitives','',f'Real prepared rows: `{a.value_count}`. Parents: `AMT_INSTALMENT`, `AMT_PAYMENT`. CT−CT is encrypted `PAYMENT_DIFF`. Public encoding scale: `{scale}`.','',f"One shared CKKS context/key setup: `{float(execution['setup_seconds']):.9f}` s. The multiply-capable context uses depth `{a.ckks_mul_depth}` (HEIR translation default: `{original_mul_depth}`); first modulus: `{emitted_multiply_parameters['first_mod_size']}`; scaling modulus: `{emitted_multiply_parameters['scaling_mod_size']}`. Setup is reported once and excluded from per-operation slowdown.",'', '| Operation | Python calculation median (s) | HE encryption median (s) | HE evaluation median (s) | HE audit decryption median (s) | HE online median (s) | Eval ÷ Python | Online ÷ Python | Max absolute error |','|---|---:|---:|---:|---:|---:|---:|---:|---:|']
