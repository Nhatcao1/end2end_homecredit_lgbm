...
def main():
 ...
 mul_cpp=work/'mul_output.cpp';patched_mul,original_mul_depth=patch_multiply_depth(mul_cpp.read_text(encoding='utf-8'),a.ckks_mul_depth);mul_cpp.write_text(patched_mul,encoding='utf-8')
 (work/'primitive_runner.cpp').write_text(RUNNER.replace('@SLOTS@','8192'),encoding='utf-8');(work/'CMakeLists.txt').write_text(CMAKE,encoding='utf-8');build=work/'build';run(['cmake','-S',str(work),'-B',str(build),f'-DOpenFHE_DIR={a.openfhe_dir}'],work);run(['cmake','--build',str(build),'--target','primitive_runner'],work);he=root/'heir_results.csv';meta=root/'execution.json';wall,log=run(['env','OMP_NUM_THREADS=1',str((build/'primitive_runner').resolve()),str(stage.resolve()),str(scale),str(a.repetitions),str(he.resolve()),str(meta.resolve())],work);(work/'runner.log').write_text(log,encoding='utf-8');rows=list(csv.DictReader(he.open()));python_rows=list(csv.DictReader(python_csv.open()));execution=json.loads(meta.read_text(encoding='utf-8'));execution['multiply_context_budget']={'translated_depth_before_patch':original_mul_depth,'requested_multiplicative_depth':a.ckks_mul_depth};write_json(meta,execution);lines=['# Real installments CKKS primitives','',f'Real prepared rows: `{a.value_count}`. Parents: `AMT_INSTALMENT`, `AMT_PAYMENT`. CT−CT is encrypted `PAYMENT_DIFF`. Public encoding scale: `{scale}`.','',f"One shared CKKS context/key setup: `{float(execution['setup_seconds']):.9f}` s. The multiply-capable context uses depth `{a.ckks_mul_depth}` (HEIR translation default: `{original_mul_depth}`). Setup is reported once and excluded from per-operation slowdown.",'', '| Operation | Python calculation median (s) | HE encryption median (s) | HE evaluation median (s) | HE audit decryption median (s) | HE online median (s) | Eval ÷ Python | Online ÷ Python | Max absolute error |','|---|---:|---:|---:|---:|---:|---:|---:|---:|']
