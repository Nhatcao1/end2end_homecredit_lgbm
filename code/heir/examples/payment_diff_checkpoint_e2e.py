...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "client-private fixed-width group CSV with VALID_MASK; bypasses "
            "PSI/group selection"
        ),
    )
    ...
    if args.audit_aggregate_checkpoint is not None:
        ...
        return
    if (args.bridge_dir is None) == (args.prepared_group is None):
        parser.error("provide exactly one of --bridge-dir or --prepared-group")
    if args.prepared_group is not None:
        prepared = load_prepared_allowed_group(
            args.prepared_group.resolve()
        )
        if prepared.bucket_size != args.bucket_size:
            raise ValueError(
                f"prepared group width {prepared.bucket_size} does not "
                f"match --bucket-size {args.bucket_size}"
            )
        # VALID_MASK is structural in this simple fixed-count proof: mask-one
        # lanes become the real encrypted parents and mask-zero lanes become
        # the zero padding generated by the HEIR tensor packer. The public
        # count fixes the reduction boundary; no invalid lane is aggregated.
        group = prepared.group
        raw_applicant_id = prepared.raw_applicant_id
    else:
        # Compatibility mode: client-only post-PSI semi-join and grouping.
