...
class OpenFHEDirectApiBenchmarkTest(unittest.TestCase):
    ...
    def test_benchmark_uses_only_the_direct_session(self):
        ...
        self.assertIn("evaluator.sum(", source)
        self.assertIn("evaluator.mean(", source)
        self.assertIn("evaluator.variance_components(", source)
        self.assertIn("client.encrypt(", source)
        self.assertIn("client.decrypt(", source)
