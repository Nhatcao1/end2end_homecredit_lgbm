...
from code.openfhe_direct.benchmarks.synthetic_vnd.subtract import (
    run_subtract_matrix,
)
from tests.test_openfhe_direct_credit_api import _OpenFHE


class OpenFHEDirectSyntheticVndSubtractTest(unittest.TestCase):
    ...
    def test_ct_subtract_reports_latency_and_accuracy(self):
        def factory(**kwargs):
    ...
    def test_benchmark_calls_session_only(self):
        source = (
            ...
            / "code/openfhe_direct/benchmarks/synthetic_vnd/subtract.py"
        ).read_text(encoding="utf-8")

        self.assertIn("OpenFHECreditSession", source)
        self.assertNotIn("import openfhe", source)
        self.assertNotIn("EvalSub", source)
