...
| `ModuleNotFoundError: No module named 'openfhe'` in the checkpoint example | The checkpoint MAX route now uses the official OpenFHE Python package. | Run `./scripts/setup_heir_openfhe_python.sh`, activate `.venv-heir-python`, and do not add `/usr/local/lib/OpenFHE` to `LD_LIBRARY_PATH`. The benchmark no longer compiles a local C++ MAX runner. |
