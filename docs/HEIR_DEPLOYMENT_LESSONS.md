...
| `ModuleNotFoundError: No module named 'openfhe'` in the checkpoint example | The example accidentally selected the optional pip Python wrapper even though this server uses a source-built OpenFHE development install. | Do not install another OpenFHE into `.venv-heir`. Use the compiled MAX route with `--openfhe-dir /usr/local/lib/OpenFHE`; Python remains the orchestrator and CMake links the existing server build. |
