...
## Persistent official-Python SUM checkpoint

`code.heir.python_api.checkpoint` adds native OpenFHE binary persistence to the
pybind module generated by the official HEIR compiler. It does not replace
HEIR arithmetic with handwritten C++.

The checkpoint is split by trust boundary:
...
The evaluator can reload `public/` and continue compatible encrypted
evaluation. Only the data owner receives `client_private/`. Saving a
ciphertext is serialization, not decryption.

These commands intentionally create a real process boundary:

```bash
python3 code/heir/scripts/save_official_heir_sum_checkpoint.py \
  --checkpoint-dir benchmark_runs/official_heir_sum_checkpoint \
  --width 8 \
  --values 160 -100 0 60 250 \
  --overwrite

python3 code/heir/scripts/load_official_heir_sum_checkpoint.py \
  --checkpoint-dir benchmark_runs/official_heir_sum_checkpoint
```
